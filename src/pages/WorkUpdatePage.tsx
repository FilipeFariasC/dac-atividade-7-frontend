import {Component, FormEvent, useState, useEffect} from "react";
...
export default function WorkUpdatePage(){
  ...
  useEffect(() => {
    if(!workLoad){
      const data = async ()=>{
        return await workService.findById(Number(id))
      };
      data().then(work => {
        setWorkTitle(work.title);
        setPublicationDate(getDate(work.publicationDate.toString()));
        setAuthorId(work.authorId);
        setAuthorByIndex(work.authorId);
        setGenre(work.genre);
        setWorkLoad(true);
      });
    } 
  ...
  async function setAuthorByIndex(index: number){
    ...
    authorValue.birthDate = getDate(authorValue.birthDate.toString());
